...
async def main():
    df = pd.read_csv("recrawl/new/nmai/recrawl_final_nmai_view.csv")
    failed_rows = []
    updated_count = 0
            ...
            if pd.isna(row["vid_nview"]) or str(row["vid_nview"]).strip() == "":
                ...
                if views:
                    ...
                    if updated_count % SAVE_EVERY == 0:
                        df.to_csv("recrawl/new/nmai/recrawl_final_nmai_view_filled.csv", index=False, encoding="utf-8-sig")
