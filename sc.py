...
class TiktokScraper:
    ...
    def __init__(self, output_path='finalProject/data/tiktok_feed.csv'):
        self.output_path = output_path

    def safe_strip(self, value):
        return value.strip() if value else ""

    def get_post_time(self, video_id: str) -> str:
        try:
    ...
    async def get_user_followers(self, context, username):
        ...
        try:
            ...
            if follower_elem:
                followers = self.safe_strip(await follower_elem.text_content())
                return followers
        except Exception as e:
    ...
    async def get_video_views(self, context, username, target_video_id):
        page = await context.new_page()
        try:
            await page.goto(f"https://www.tiktok.com/@{username}", timeout=30000)
            ...
            for card in video_cards:
                ...
                if target_video_id in video_url:
                    ...
                    views = self.safe_strip(await views_elem.text_content()) if views_elem else ""
                    return views
        except Exception as e:
    ...
    async def scrape_feed(self, num_scrolls=10):
        async with async_playwright() as p:
            ...
            for i in range(num_scrolls):
                ...
                try:
                    ...
                    username = self.safe_strip(await username_elem.text_content() if username_elem else "")
                    likes = self.safe_strip(await likes_elem.text_content() if likes_elem else "")
                    comments = self.safe_strip(await comments_elem.text_content() if comments_elem else "")
                    shares = self.safe_strip(await shares_elem.text_content() if shares_elem else "")
                    undefined = self.safe_strip(await undefined_elem.text_content() if undefined_elem else "")
                    caption = self.safe_strip(await caption_elem.text_content() if caption_elem else "")
                    ...
                    views = await self.get_video_views(context, username, video_id)
                    followers = await self.get_user_followers(context, username)
                    post_time = self.get_post_time(video_id)
                    ...
                    if music_href:
                        ...
                        if href:
                            ...
                            try:
                                ...
                                if music_title_elem:
                                    sound_title = self.safe_strip(await music_title_elem.text_content())

                                uses_elem = await sound_page.query_selector('h2[data-e2e="music-video-count"] strong')
                                if uses_elem:
                                    uses_sound_count = self.safe_strip(await uses_elem.text_content())
                                ...
                                for author_elem in music_author_elems:
                                    display_name = self.safe_strip(await author_elem.text_content())
                        ...
                        if music_text_div:
                            ...
                            if music_text:
                                sound_title = self.safe_strip(music_text)

                    results.append({
                        "user_name": username,
                        "user_nfollower": followers,
                        "vid_id": self.safe_strip(video_id),
                        "vid_caption": caption,
                        ...
                        "vid_url": self.safe_strip(video_url),
                        "music_id": sound_id,
    ...
    def save_to_csv(self, data):
        with open(self.output_path, mode="w", newline='', encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=[
                "user_name", "user_nfollower", "vid_id", "vid_caption", "vid_postTime", "vid_scrapeTime", 
        ...
        print(f"\nĐã lưu {len(data)} video vào file {self.output_path}")


if __name__ == "__main__":
    scraper = TiktokScraper()
    try:
        data = asyncio.run(scraper.scrape_feed(num_scrolls=5))
    except Exception as e:
